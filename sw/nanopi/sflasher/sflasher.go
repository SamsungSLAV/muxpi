...
func (sf *Sflasher) computeHash(reader io.Reader) (chan []byte, *io.PipeReader) {
	...
	go func() {
		...
		if err != nil {
			logger.WithError(err).Error("Failed to copy data from Reader to PipeReader.")
...
func (sf *Sflasher) DownloadAndFlash(md5sums string, urls ...string) (err error) {
	...
	for _, u := range urls {
		...
		if err != nil {
			logger.WithError(err).Errorf("Failed to download and flash: %s.", u)
			return
		}
	}
	return
...
func (sf *Sflasher) Flash(md5sumsPath string, paths ...string) (err error) {
	...
	for _, p := range paths {
		...
		f, err = os.Open(p)
		if err != nil {
			logger.WithError(err).Errorf("Failed to open file: %s", p)
			return
		}
		err = sf.flashFromReader(f, f.Name())
		if err != nil {
			logger.WithError(err).Error("Failed to flash with: %s", p)
			return
		}
	}
	return

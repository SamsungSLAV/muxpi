...
TGT_CFLAGS	+= -fno-common -ffunction-sections -fdata-sections -save-temps
...
TGT_CXXFLAGS	+= -fno-common -ffunction-sections -fdata-sections -save-temps
...
elf: $(BINARY).elf print_size
bin: $(BINARY).bin print_size
hex: $(BINARY).hex print_size
srec: $(BINARY).srec
list: $(BINARY).list
...
clean:
	@#printf "  CLEAN\n"
	$(Q)$(RM) *.i *.s *.o *.d *.elf *.bin *.hex *.srec *.list *.map generated.* *.stylecheck ${OBJS} ${OBJS:o=d}
...
%.flash: %.bin
	$(OOCD) -f board/stm32f0discovery.cfg -c "init" -c "reset init" -c "program $(*).bin exit 0x08000000 reset"
